...
class SyncMediaCommand extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($files as $file) {
            ...
            if (! Str::contains($file, ['.jpg', '.jpeg', '.png', '.webp', '.svg', '.pdf'])) {
                continue;
            }
            // Lewati folder internal/temp
            if (Str::contains($file, ['thumbnails/', '.gitignore', 'framework/', 'sessions/'])) {
                continue;
            }
    ...
    {
        if (Str::contains($path, 'branding')) {
            return 'branding';
        }
        if (Str::contains($path, 'cms')) {
            return 'cms';
        }
        if (Str::contains($path, 'gallery')) {
            return 'gallery';
        }
        if (Str::contains($path, 'tour')) {
            return 'tour';
        }
        if (Str::contains($path, 'outbound')) {
            return 'outbound';
        }
    ...
    private function detectGalleryCategory($path)
    {
        ...
        if (Str::contains($pathLower, 'outbound')) {
            return 'outbound';
        }
        if (Str::contains($pathLower, 'tour')) {
            return 'tour';
        }
