...
@section('title', 'Galeri Foto Wisata – Sujailake Toba')
...
>
        ...
        <div class="relative z-10 w-full max-w-7xl mx-auto px-6 md:px-8 text-white pt-20">
            <div class="max-w-4xl animate-in fade-in slide-in-from-left-12 duration-1000">
                <div class="flex items-center space-x-3 mb-8">
                    ...
                    <span class="text-lake-light font-black text-xs uppercase tracking-[0.4em]">Visual Storytelling</span>
                </div>
                <h1 class="text-5xl md:text-8xl font-black tracking-tighter leading-[0.9] mb-10">
                    Galeri <br />
                    <span class="text-lake-blue">Momen Indah</span>
                </h1>
        ...
        <div class="bg-white rounded-[3.5rem] shadow-[0_50px_100px_-20px_rgba(15,23,42,0.1)] p-8 md:p-12 border border-slate-100 animate-in fade-in zoom-in duration-1000 delay-300">
            <div class="flex flex-col lg:flex-row items-center justify-between gap-10">
                ...
                <div class="relative group w-full lg:w-96">
                    <div class="absolute left-6 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-lake-blue transition-colors pointer-events-none">
                    ...
                    <input
                        ...
                        x-model="searchQuery"
                        class="w-full pl-16 pr-8 py-5 bg-slate-50 border-none rounded-2xl focus:ring-4 focus:ring-lake-blue/10 font-bold text-slate-900 placeholder:text-slate-400 text-sm transition-all"
                    >
                </div>
            </div>
        </div>
    ...
    <div class="max-w-7xl mx-auto px-6 md:px-8 mt-24">
        <div class="flex items-center justify-between mb-16">
            <div>
                <h2 class="text-4xl md:text-5xl font-black text-slate-900 tracking-tighter">Koleksi <span class="text-lake-blue">Visual</span></h2>
                <p class="text-slate-500 font-medium text-sm mt-2">
                    Menampilkan <span class="text-lake-blue font-black" x-text="filteredImages.length"></span> mahakarya alam Sumatera Utara
                </p>
            </div>
        </div>
        ...
        <div class="columns-1 md:columns-2 lg:columns-3 xl:columns-4 gap-8 space-y-8">
            <template x-for="(img, index) in filteredImages" :key="index">
                <div
                    class="break-inside-avoid relative rounded-[2.5rem] overflow-hidden group cursor-pointer shadow-xl transition-all duration-700 hover:-translate-y-2 hover:shadow-lake-blue/20 border border-slate-50"
                ...
                >
                    <img
                        :src="img.image_url"
                        :alt="img.caption || 'Galeri Sujailake Toba'"
                    ...
                    <div class="absolute inset-0 bg-gradient-to-t from-slate-900/90 via-slate-900/20 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-700 flex flex-col justify-end p-8">
                        <div class="transform translate-y-8 group-hover:translate-y-0 transition-all duration-700">
                            <span x-show="img.category" class="inline-block px-4 py-1.5 bg-lake-blue text-white text-[10px] font-black uppercase tracking-[0.2em] rounded-xl mb-4 shadow-lg shadow-lake-blue/30" x-text="img.category"></span>
    ...
    <template x-if="lightbox.open">
        ...
        >
            ...
            <div class="absolute top-10 left-10 bg-white/5 backdrop-blur-xl border border-white/10 px-8 py-4 rounded-[1.5rem] text-white text-[10px] font-black uppercase tracking-[0.4em] z-[210] hidden md:block">
                IMAGE <span x-text="lightbox.index + 1" class="text-lake-light text-lg"></span> OF <span x-text="filteredImages.length" class="text-lg opacity-40"></span>
            </div>
            ...
            <div class="absolute inset-x-0 top-1/2 -translate-y-1/2 flex justify-between px-8 md:px-12 pointer-events-none z-[210]">
                <button
                    @click.stop="prev()"
                    class="w-20 h-20 bg-white/5 hover:bg-lake-blue backdrop-blur-xl rounded-[2rem] flex items-center justify-center text-white transition-all pointer-events-auto border border-white/10 shadow-2xl group"
                >
                ...
                <div class="relative max-w-6xl w-full h-full flex flex-col items-center justify-center p-8 md:p-20">
                    ...
                    <div class="mt-12 text-center max-w-3xl animate-in fade-in slide-in-from-bottom-8 duration-700">
                        <div class="flex items-center justify-center gap-3 mb-4">
                            <span x-text="filteredImages[lightbox.index].category" class="px-5 py-2 bg-lake-blue text-white text-[10px] font-black uppercase tracking-[0.3em] rounded-full shadow-lg shadow-lake-blue/20"></span>
                        </div>
                        ...
                        <template x-if="filteredImages[lightbox.index].type === 'package' || filteredImages[lightbox.index].type === 'blog'">
                            <div class="mt-8">
                                <a :href="filteredImages[lightbox.index].type === 'package' ? '/package/' + filteredImages[lightbox.index].slug : '/blog/' + filteredImages[lightbox.index].slug" 
                                   class="inline-flex items-center gap-3 px-8 py-4 bg-white text-slate-900 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-lake-blue hover:text-white transition-all shadow-2xl">
