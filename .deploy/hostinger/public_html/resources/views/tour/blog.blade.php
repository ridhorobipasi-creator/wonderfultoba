...
@section('title', 'Blog & Inspirasi Wisata – Sujailake Toba')
        ...
        <div class="relative z-10 w-full max-w-7xl mx-auto px-6 md:px-8 text-white pt-20">
            <div class="max-w-4xl animate-in fade-in slide-in-from-left-12 duration-1000">
                <div class="flex items-center space-x-3 mb-8">
                    ...
                    <span class="text-lake-light font-black text-xs uppercase tracking-[0.4em]">Jurnal Perjalanan</span>
                </div>
                <h1 class="text-5xl md:text-8xl font-black tracking-tighter leading-[0.9] mb-10">
                    Inspirasi <br />
                    <span class="text-lake-blue">Eksplorasi</span>
                </h1>
        ...
        <template x-if="featured">
            <article class="group relative bg-slate-900 rounded-[4rem] overflow-hidden shadow-2xl transition-all duration-700 mb-24 min-h-[500px] md:min-h-[600px] flex flex-col justify-end">
                ...
                <div class="relative z-10 p-10 md:p-20 max-w-4xl animate-in fade-in slide-in-from-bottom-12 duration-1000">
                    <div class="flex items-center gap-6 text-lake-light text-xs font-black uppercase tracking-[0.4em] mb-8">
                        <span class="px-4 py-1.5 bg-lake-blue text-white rounded-full" x-text="featured.category"></span>
                    ...
                    <h2 class="text-4xl md:text-7xl font-black text-white mb-8 group-hover:text-lake-blue transition-colors leading-[1.05] tracking-tighter" x-text="featured.title"></h2>
                    ...
                    <a :href="'/blog/' + (featured.slug || featured.id)" class="inline-flex items-center gap-4 bg-white text-slate-900 px-10 py-5 rounded-[1.5rem] font-black text-xs uppercase tracking-[0.2em] hover:bg-lake-blue hover:text-white transition-all duration-500 shadow-2xl group/btn">
                        Baca Cerita Lengkap
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12 lg:gap-16">
            <template x-for="(post, i) in rest" :key="post.id">
                <article class="group flex flex-col h-full animate-in fade-in slide-in-from-bottom-8 duration-1000" :style="'animation-delay: ' + (i * 100) + 'ms'">
                    <a :href="'/blog/' + (post.slug || post.id)" class="block mb-8 relative overflow-hidden rounded-[2.5rem] shadow-xl h-72">
                    ...
                    <div class="flex-1 flex flex-col px-2">
                        <div class="flex items-center gap-3 text-lake-blue text-[10px] font-black uppercase tracking-[0.3em] mb-4">
                            <span x-text="new Date(post.createdAt).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })"></span>
                        </div>
                        <h2 class="text-2xl font-black text-slate-900 mb-4 group-hover:text-lake-blue transition-colors leading-tight line-clamp-2 tracking-tight" x-text="post.title"></h2>
                        ...
                        <div class="pt-8 border-t border-slate-100">
                            <a :href="'/blog/' + (post.slug || post.id)" class="flex items-center gap-3 text-slate-900 font-black text-[10px] uppercase tracking-widest group/link">
                                Baca Selengkapnya
                                <svg class="w-4 h-4 text-lake-blue group-hover/link:translate-x-2 transition-transform" fill="none" stroke="currentColor" stroke-width="3" viewBox="0 0 24 24"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
                            </a>
                        </div>
                    </div>
                </article>
