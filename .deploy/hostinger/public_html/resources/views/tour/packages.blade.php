...
@section('title', 'Paket Wisata Sumatera Utara – Sujailake Toba')
...
<div 
    x-data="{ 
        ...
        cities: {{ \Illuminate\Support\Js::from($cities) }},
...
>
    <main class="flex-grow">
        ...
        <div class="relative h-[65vh] flex items-end overflow-hidden">
            @php
                $heroImg = !empty($siteSettings['cms_tour']['hero_image']) 
                    ? imageUrl($siteSettings['cms_tour']['hero_image']) 
            ...
            <div class="relative z-10 w-full max-w-7xl mx-auto px-4 pb-32">
                <div class="animate-in fade-in slide-in-from-bottom-8 duration-1000">
                    <span class="inline-block px-4 py-1.5 bg-lake-blue text-white text-[10px] font-bold uppercase tracking-[0.3em] rounded-full mb-4">Eksplorasi Indonesia & Dunia</span>
                    <h1 class="text-5xl md:text-7xl font-black text-white tracking-tight leading-tight">
                        Paket Wisata <span class="text-lake-light">Pilihan</span><br>Terbaik
                    </h1>
                </div>
            </div>
        </div>
            ...
            <div class="bg-white/90 backdrop-blur-2xl p-6 md:p-8 rounded-[2.5rem] shadow-2xl shadow-slate-200/60 border border-white/60 animate-in fade-in zoom-in duration-1000 delay-300">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div class="relative group lg:col-span-2">
                        <div class="absolute left-5 top-1/2 -translate-y-1/2 w-9 h-9 bg-slate-100 rounded-xl flex items-center justify-center text-slate-400 group-focus-within:bg-lake-blue group-focus-within:text-white transition-all shadow-sm">
                        ...
                        <input type="text" placeholder="Cari destinasi atau paket..." x-model="searchQuery"
                            class="w-full pl-16 pr-5 py-4 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-lake-blue/20 font-medium text-slate-700 placeholder:text-slate-400">
                    </div>
                    <!-- City Filter -->
                    <div class="relative group">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-map-pin absolute left-5 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-lake-blue pointer-events-none"><path d="M20 10c0 4.993-5.539 10.193-7.399 11.799a1 1 0 0 1-1.202 0C9.539 20.193 4 14.993 4 10a8 8 0 0 1 16 0"></path><circle cx="12" cy="10" r="3"></circle></svg>
                        <select x-model="filterCity"
                            class="w-full pl-12 pr-4 py-4 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-lake-blue/20 font-medium text-slate-700 appearance-none cursor-pointer">
                    ...
                    <!-- Sort Filter -->
                    <div class="relative group">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-sliders-horizontal absolute left-5 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-lake-blue pointer-events-none"><path d="m21 16-4 4-4-4"></path><path d="M17 20V4"></path><path d="m3 8 4-4 4 4"></path><path d="M7 4v16"></path></svg>
                ...
                <div class="flex flex-wrap gap-2 mt-4">
                    ...
                    <button @click="filterDuration = '{{ $d }}'"
                        :class="filterDuration === '{{ $d }}' ? 'bg-lake-blue text-white shadow-lg shadow-lake-blue/20' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'"
        ...
        <div class="max-w-7xl mx-auto px-4 mt-14">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
                <div>
                    ...
                    <p class="text-slate-500 font-medium text-sm">Ditemukan <span class="text-lake-blue font-black" x-text="filteredPackages.length"></span> paket wisata</p>
                </div>
            </div>
                        ...
                        <div class="bg-white rounded-[2rem] overflow-hidden border border-slate-100 hover:shadow-[0_30px_60px_-15px_rgba(0,0,0,0.1)] transition-all duration-500 group h-full flex flex-col">
                            ...
                            <div class="p-8 flex flex-col flex-grow">
                                ...
                                <div class="flex items-center justify-between pt-6 border-t border-slate-50">
                                    ...
                                    <a :href="'/package/' + (pkg.slug || pkg.id)" class="w-14 h-14 bg-slate-900 text-white rounded-2xl flex items-center justify-center hover:bg-lake-blue transition-all shadow-xl shadow-slate-200 group/btn" aria-label="Lihat detail">
            ...
            <div x-show="filteredPackages.length === 0" class="text-center py-40 bg-slate-50 rounded-[4rem] border-2 border-dashed border-slate-200 animate-in fade-in zoom-in duration-700">
                ...
                <button @click="searchQuery = ''; filterCity = 'all'; filterDuration = 'Semua'; sortBy = 'default'"
                    class="bg-slate-900 text-white px-12 py-6 rounded-2xl font-black text-xs uppercase tracking-widest shadow-2xl hover:bg-lake-blue transition-all duration-500">
                    Reset Semua Filter
                </button>
            </div>
        </div>
        ...
        <div class="max-w-7xl mx-auto px-4 mt-24 mb-24">
            <div class="bg-gradient-to-r from-lake-blue to-lake-mid rounded-[2.5rem] p-10 md:p-14 text-center relative overflow-hidden">
                ...
                <div class="relative z-10">
                    ...
                    <div class="flex flex-col sm:flex-row gap-4 justify-center">
                        <a href="tel:+{{ preg_replace('/[^0-9]/', '', $siteSettings['general']['whatsapp'] ?? '6281323888207') }}" class="flex items-center justify-center gap-2 bg-white text-lake-blue px-8 py-4 rounded-2xl font-black hover:bg-slate-50 transition-all shadow-xl">
