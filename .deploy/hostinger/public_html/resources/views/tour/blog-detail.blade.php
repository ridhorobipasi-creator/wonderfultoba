...
@section('title', ($post->title ?? 'Blog') . ' – Sujailake Toba')
...
}" @scroll.window="updateProgress()" class="bg-white min-h-screen pb-32">
    <!-- Reading Progress Bar -->
    <div class="fixed top-0 left-0 w-full h-1 z-[100] pointer-events-none">
        <div class="h-full bg-lake-blue transition-all duration-150" :style="`width: ${progress}%`"></div>
    </div>
    ...
    <div class="relative h-[70dvh] w-full overflow-hidden bg-slate-900">
        ...
        <div class="relative z-10 h-full max-w-5xl mx-auto px-6 md:px-8 flex flex-col justify-center text-center md:text-left">
            <div class="animate-in fade-in slide-in-from-bottom-12 duration-1000">
                <a href="/blog" class="inline-flex items-center gap-3 text-white/80 hover:text-white font-black text-[10px] uppercase tracking-[0.4em] mb-10 transition-all group">
                ...
                <div class="flex flex-wrap items-center justify-center md:justify-start gap-4 mb-8">
                    <span class="px-5 py-2 bg-lake-blue text-white rounded-full text-[10px] font-black uppercase tracking-widest shadow-xl shadow-lake-blue/20">
                        {{ $post->category }}
                    </span>
            ...
            <div class="lg:col-span-8">
                <article class="bg-white rounded-[3rem] p-10 md:p-20 shadow-[0_50px_100px_-20px_rgba(15,23,42,0.08)] border border-slate-50">
                    ...
                    <div class="flex items-center gap-4 mb-16 pb-10 border-b border-slate-50">
                        ...
                        <div>
                            <p class="text-[10px] font-black text-lake-blue uppercase tracking-[0.3em] mb-1">Ditulis Oleh</p>
                            <p class="text-lg font-black text-slate-900 tracking-tight">Tim Redaksi Sujailake Toba</p>
                        </div>
                    </div>
                    ...
                    @if(isset($post->tags) && count($post->tags) > 0)
                        <div class="mt-20 pt-10 border-t border-slate-50 flex flex-wrap gap-3">
                            @foreach($post->tags as $tag)
                                <span class="px-6 py-2 bg-slate-50 text-slate-500 rounded-full text-[10px] font-black uppercase tracking-widest border border-slate-100 hover:bg-lake-blue hover:text-white transition-all cursor-default">#{{ $tag }}</span>
                            @endforeach
                        </div>
                    @endif
                    ...
                    <div class="mt-16 p-10 bg-slate-50 rounded-[2.5rem] border border-slate-100 flex flex-col md:flex-row items-center justify-between gap-8">
                        ...
                        <div class="flex items-center gap-4">
                            <a href="https://wa.me/?text={{ urlencode('Baca artikel ini dari Sujailake Toba: ' . $post->title . ' ' . url()->current()) }}" 
                               target="_blank"
                ...
                <div class="sticky top-28 space-y-8 animate-in fade-in slide-in-from-right-12 duration-1000">
                    ...
                    <div class="bg-slate-900 rounded-[3.5rem] p-10 md:p-12 text-white shadow-[0_50px_100px_-20px_rgba(15,23,42,0.4)] relative overflow-hidden">
                        ...
                        <div class="relative z-10">
                            <span class="text-lake-light font-black text-[11px] uppercase tracking-[0.4em] mb-6 block">Eksplorasi Sekarang</span>
                            ...
                            <a href="/packages" class="w-full flex items-center justify-center gap-3 py-6 bg-lake-blue text-white rounded-[2rem] font-black text-sm uppercase tracking-[0.2em] hover:bg-toba-accent hover:text-slate-900 transition-all duration-500 shadow-2xl shadow-lake-blue/20 group">
                                Lihat Paket Wisata
                    ...
                    <div class="bg-white rounded-[3rem] p-10 border border-slate-100 shadow-xl shadow-slate-200/50">
                        ...
                        <div class="flex items-center gap-4 mb-6">
                            <div class="w-14 h-14 rounded-2xl bg-lake-blue/10 text-lake-blue flex items-center justify-center">
                            ...
                            <div>
                                <p class="font-black text-slate-900">Sujailake Toba</p>
                                <p class="text-[9px] font-black text-lake-blue uppercase tracking-widest">Editorial Team</p>
                            </div>
                        </div>
        ...
        @if(count($relatedPosts) > 0)
            <div class="mt-32">
                <div class="flex items-center space-x-3 mb-12">
                    ...
                    <span class="text-lake-blue font-black text-xs uppercase tracking-[0.4em]">Inspirasi Lainnya</span>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-12">
                    @foreach($relatedPosts as $rp)
                        <article class="group">
                            <a href="/blog/{{ $rp->slug ?? $rp->id }}" class="block">
                                ...
                                <div class="px-2">
                                    <span class="text-[10px] font-black text-lake-blue uppercase tracking-[0.3em] mb-3 block">{{ $rp->category }}</span>
                                    <h4 class="text-xl font-black text-slate-900 mb-4 group-hover:text-lake-blue transition-colors leading-tight tracking-tight line-clamp-2">{{ $rp->title }}</h4>
                                    <div class="flex items-center gap-2 text-slate-900 font-black text-[9px] uppercase tracking-widest group-hover:gap-4 transition-all">
                                        Selanjutnya
                                        <svg class="w-4 h-4 text-lake-blue" fill="none" stroke="currentColor" stroke-width="3" viewBox="0 0 24 24"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
                                    </div>
                                </div>
                            </a>
                        </article>
