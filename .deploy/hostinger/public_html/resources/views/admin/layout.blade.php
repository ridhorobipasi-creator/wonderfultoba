...
<head>
    ...
    <title>@yield('title', 'Dashboard') - Sujailake Toba Admin</title>
    @php
...
<body class="antialiased text-slate-900">
        ...
        >
            <div class="flex flex-col h-full">
                ...
                <div class="px-6 pt-8 pb-6 flex flex-col items-center text-center flex-shrink-0 border-b border-slate-50 relative">
                    ...
                    @else
                        <div class="w-12 h-12 bg-lake-blue rounded-2xl flex items-center justify-center shadow-lg shadow-lake-blue/30 mb-3">
                        ...
                        <h1 class="text-base font-black text-slate-900 tracking-tight leading-tight mb-3">
                            {{ $siteSettings['general']['site_name'] ?? ($siteSettings['cms_landing']['brand_name'] ?? 'Sujailake Toba') }}
                        </h1>
                    @endif
                    <a href="{{ route('index') }}" target="_blank"
                       class="flex items-center gap-2 px-4 py-2 bg-slate-50 text-slate-400 hover:text-lake-blue rounded-xl text-[10px] font-black uppercase tracking-widest transition">
                    ...
                    <div class="mb-5">
                        ...
                        <a href="{{ route('admin.dashboard') }}"
                           class="flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all font-bold text-[13px]
                                  {{ request()->routeIs('admin.dashboard') ? 'bg-lake-blue text-white shadow-lg shadow-lake-blue/20' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900' }}">
                            <i class="fas fa-chart-line w-5 text-sm {{ request()->routeIs('admin.dashboard') ? 'text-white' : 'text-lake-blue' }}"></i>
                            Dashboard
                        </a>
                    </div>
                    ...
                    <div class="mb-5">
                        ...
                        <div class="space-y-0.5">
                            <a href="{{ route('admin.bookings.index') }}"
                               class="flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all font-bold text-[13px]
                                      {{ request()->routeIs('admin.bookings.*') ? 'bg-lake-blue text-white shadow-lg shadow-lake-blue/20' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900' }}">
                                <i class="fas fa-clipboard-list w-5 text-sm {{ request()->routeIs('admin.bookings.*') ? 'text-white' : 'text-lake-light' }}"></i>
                                Daftar Pesanan
                            </a>

                            <a href="{{ route('admin.customers.index') }}"
                               class="flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all font-bold text-[13px]
                                      {{ request()->routeIs('admin.customers.*') ? 'bg-lake-blue text-white shadow-lg shadow-lake-blue/20' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900' }}">
                            ...
                            <a href="{{ route('admin.reports.financial') }}"
                               class="flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all font-bold text-[13px]
                                      {{ request()->routeIs('admin.reports.financial') ? 'bg-lake-blue text-white shadow-lg shadow-lake-blue/20' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900' }}">
                    ...
                    <div class="mb-5">
                        ...
                        <div class="space-y-0.5">
                            ...
                            <a href="{{ route('admin.cms.index') }}"
                               class="flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all font-bold text-[13px]
                                      {{ request()->routeIs('admin.cms.index') ? 'bg-lake-blue text-white shadow-lg shadow-lake-blue/20' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900' }}">
                            ...
                            <a href="{{ route('admin.blogs.index') }}"
                               class="flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all font-bold text-[13px]
                                      {{ request()->routeIs('admin.blogs.*') ? 'bg-lake-blue text-white shadow-lg shadow-lake-blue/20' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900' }}">
                                <i class="fas fa-pen-nib w-5 text-sm {{ request()->routeIs('admin.blogs.*') ? 'text-white' : 'text-lake-light' }}"></i>
                                Blog / Artikel
                            </a>
                             @if(auth()->user()->isSuperAdmin())
                             <a href="{{ route('admin.cities.index') }}"
                                class="flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all font-bold text-[13px]
                                       {{ request()->routeIs('admin.cities.*') || request()->routeIs('admin.regencies.*') ? 'bg-lake-blue text-white shadow-lg shadow-lake-blue/20' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900' }}">
                             ...
                             <a href="{{ route('admin.gallery.index', ['category' => 'tour']) }}"
                                class="flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all font-bold text-[13px]
                                       {{ request()->routeIs('admin.gallery.*') && request('category', 'tour') == 'tour' ? 'bg-lake-blue text-white shadow-lg shadow-lake-blue/20' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900' }}">
                             ...
                             <a href="{{ route('admin.cms.pages') }}"
                                class="flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all font-bold text-[13px]
                                       {{ request()->routeIs('admin.cms.pages') ? 'bg-lake-blue text-white shadow-lg shadow-lake-blue/20' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900' }}">
                    ...
                    <div class="mb-5">
                        ...
                        <div class="space-y-0.5">
                            <a href="{{ route('admin.packages.index') }}"
                               class="flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all font-bold text-[13px]
                                      {{ request()->routeIs('admin.packages.*') ? 'bg-lake-blue text-white shadow-lg shadow-lake-blue/20' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900' }}">
                                 <i class="fas fa-map w-5 text-sm {{ request()->routeIs('admin.packages.*') ? 'text-white' : 'text-lake-light' }}"></i>
                                 Paket Tour Wisata
                            </a>
                            <a href="{{ route('admin.cars.index') }}"
                               class="flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all font-bold text-[13px]
                                      {{ request()->routeIs('admin.cars.*') ? 'bg-lake-blue text-white shadow-lg shadow-lake-blue/20' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900' }}">
                                 <i class="fas fa-car w-5 text-sm {{ request()->routeIs('admin.cars.*') ? 'text-white' : 'text-blue-500' }}"></i>
                                 Armada Sewa Mobil
                            </a>
                            <a href="{{ route('admin.clients.index') }}"
                               class="flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all font-bold text-[13px]
                                      {{ request()->routeIs('admin.clients.*') ? 'bg-lake-blue text-white shadow-lg shadow-lake-blue/20' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900' }}">
                                 <i class="fas fa-building w-5 text-sm {{ request()->routeIs('admin.clients.*') ? 'text-white' : 'text-violet-500' }}"></i>
                                 Klien & Mitra
                            </a>
                        </div>
                    </div>
        ...
        <div id="content-area"
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <div class="hidden sm:flex items-center gap-1.5 text-[8px] font-black uppercase tracking-widest text-slate-300 mb-1">
                            <a href="{{ route('admin.dashboard') }}" class="hover:text-lake-blue transition">Admin</a>
                    ...
                    <div class="hidden md:flex items-center ml-8 relative group" x-data="{ commandOpen: false }">
                        <div class="relative">
                             <i class="fas fa-magnifying-glass absolute left-5 top-1/2 -translate-y-1/2 text-slate-300 text-[10px] group-focus-within:text-lake-blue transition-colors"></i>
                            <input type="text" 
                                   @focus="commandOpen = true"
                                   @click.away="commandOpen = false"
                                   placeholder="Cari Pesanan, Paket, atau Berita... (CTRL+K)"                                    class="w-80 lg:w-[400px] pl-12 pr-12 py-3 bg-slate-50 border-none rounded-[1.2rem] text-[11px] font-bold text-slate-600 placeholder:text-slate-300 focus:ring-2 focus:ring-lake-blue/20 focus:bg-white transition-all shadow-sm group-hover:shadow-md">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div class="relative" x-data="{ open: false }">
                        <button @click="open = !open" @click.away="open = false" 
                            ...
                            <div class="w-8 h-8 rounded-xl bg-slate-900 text-white flex items-center justify-center text-[10px] font-black group-hover:bg-lake-blue transition">
                ...
                <div class="fixed top-24 right-8 z-[9999] flex flex-col gap-4 w-80 pointer-events-none">
                    @if(session('success'))
                        <div x-data="{ show: true }"
                            ...
                            <div class="absolute top-0 left-0 w-1.5 h-full bg-lake-blue"></div>
                            <div class="flex items-center gap-4">
                                <div class="w-12 h-12 rounded-2xl bg-lake-blue text-white flex items-center justify-center flex-shrink-0 shadow-lg shadow-lake-blue/20">
                                    <i class="fas fa-check-double text-xs"></i>
                                </div>
                                <div>
                                    <p class="text-[9px] font-black uppercase tracking-[0.2em] text-lake-blue mb-0.5">Success</p>
            ...
            <footer class="px-8 py-5 border-t border-slate-50 flex flex-col sm:flex-row items-center justify-between gap-2 flex-shrink-0">
                <p class="text-[9px] font-black text-slate-300 uppercase tracking-[0.4em]">
                    Sujailake Toba Engine &bull; Management v3.0
                </p>
                <p class="text-[9px] font-black text-slate-300 uppercase tracking-widest">
                    Crafted with <i class="fas fa-heart text-rose-400"></i> for Danau Toba Indonesia
                </p>
            </footer>
    ...
    <div x-show="loading" 
         ...
         x-cloak>
        <div class="relative">
            <div class="w-20 h-20 border-4 border-lake-blue/20 border-t-lake-blue rounded-full animate-spin"></div>
