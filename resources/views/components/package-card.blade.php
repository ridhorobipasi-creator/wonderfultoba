...
@php
    ...
    $image = $package->packageImages->first()?->image_path 
...
<div class="bg-white rounded-2xl overflow-hidden border border-slate-100 hover:shadow-[0_30px_60px_-15px_rgba(0,0,0,0.1)] transition-all duration-500 group flex flex-col h-full">
    <div class="relative h-64 md:h-72 overflow-hidden shrink-0">
        ...
        <!-- Wishlist -->
        <button
            class="absolute top-5 right-5 w-9 h-9 md:w-10 md:h-10 bg-white/20 backdrop-blur-md rounded-full flex items-center justify-center text-white hover:bg-white hover:text-rose-500 transition-all"
        >
    ...
    <div class="p-6 md:p-8 flex flex-col flex-grow">
        ...
        <h3 class="text-xl md:text-2xl font-black text-slate-900 mb-4 line-clamp-1 group-hover:text-blue-600 transition-colors tracking-tight">
            {{ $package->name }}
        </h3>
        ...
        <div class="flex items-center justify-between pt-6 border-t border-slate-50 mt-auto">
            ...
            <a
                ...
                class="w-12 h-12 md:w-14 md:h-14 bg-slate-900 text-white rounded-2xl flex items-center justify-center hover:bg-blue-600 transition-all shadow-xl shadow-slate-200 group/btn"
            >
